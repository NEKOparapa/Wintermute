...
# 构建 L1 提示词并控制 token 数量
def _build_l1_prompt_with_budget(
    memories: list[dict[str, object]],
    l0_recent_events: list[dict[str, object]],
    *,
    identity: str,
    user_profile: str,
    ...
    kept_events = list(l0_recent_events)
...
def _build_l1_prompt(
    ...
    *,
    ...
    system_parts = [
        _L1_SYSTEM_PROMPT.strip(),  # L1 默认前缀提示词块
    ...
    return PromptContent(
        ...
        messages=build_history_messages(l0_recent_events),
    )
...
# 构建当前 L1 主动触发事件块
def active_l1_event_block(event: dict[str, object] | None) -> str:
    if not event:
        return ""
